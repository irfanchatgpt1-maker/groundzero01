...
export const AdminDashboard: React.FC<{ onRoleChange: (role: UserRole) => void }> = ({ onRoleChange }) => {
  ...
  useEffect(() => {
    if (shipments.length > 0 && !selectedShipment) setSelectedShipment(shipments[0]);
  ...
  const getExportData = (): Record<string, unknown>[] => {
    switch (activeTab) {
      case 'Camps Management': return camps.map(c => ({ Name: c.name, Location: c.location, Status: c.status, Occupancy: c.occupancy, Capacity: c.capacity, Lead: c.lead }));
      case 'Global Logistics': return shipments.map(s => ({ TrackingID: s.tracking_id, Resource: s.resource, Origin: s.origin, Destination: s.destination, Status: s.status, Quantity: s.quantity, Urgency: s.urgency, ETA: s.eta }));
      case 'Resource Inventory': return inventory.map(i => ({ Item: i.item_name, Category: i.category, Quantity: i.quantity, Unit: i.unit, Threshold: i.min_threshold, Critical: i.is_critical }));
      default: return [...camps.map(c => ({ Type: 'Camp', Name: c.name, Status: c.status, Location: c.location })), ...shipments.map(s => ({ Type: 'Shipment', Name: s.resource, Status: s.status, Location: s.destination }))];
    }
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
        ...
        return (
          <motion.div key="dashboard" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
              <div>
                <h2 className="text-xl sm:text-2xl font-black text-foreground tracking-tight">System Overview</h2>
              ...
              <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
                <ExportDropdown data={getExportData()} filename="groundzero-overview" />
                <button onClick={() => setNewEntryOpen(true)} className="bg-primary text-primary-foreground px-4 sm:px-5 py-2 sm:py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-primary/20 hover:opacity-90 active:scale-95 transition-all">
                  ...
                  <tbody className="divide-y divide-border">
                    {auditLogs.map(log => (
                      <tr key={log.id} className="hover:bg-muted/50 transition-colors">
                        <td className="px-4 sm:px-6 py-4 sm:py-5 text-xs font-black text-foreground">#{log.entity_id.slice(0, 12)}</td>
                        <td className="px-4 sm:px-6 py-4 sm:py-5 text-xs font-bold text-muted-foreground">{log.entity_type}</td>
                        <td className="px-4 sm:px-6 py-4 sm:py-5"><span className="text-xs font-bold text-foreground/80">{log.action}</span></td>
                        <td className="px-4 sm:px-6 py-4 sm:py-5 text-xs font-black text-foreground hidden sm:table-cell">{log.details?.slice(0, 50)}</td>
                        <td className="px-4 sm:px-6 py-4 sm:py-5 text-right"><button className="text-[10px] font-black text-primary uppercase hover:underline tracking-widest">View</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      case 'Global Logistics':
        return (
          <motion.div key="logistics" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-7">
              <AnimatePresence mode="wait">
                {selectedShipment ? (
                  <motion.div key={selectedShipment.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} className="bg-card dark:bg-card-dark rounded-[2rem] lg:rounded-[2.5rem] border border-border shadow-xl overflow-hidden lg:sticky lg:top-24">
                    <div className="p-5 sm:p-8 border-b border-border bg-muted/50">
                      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-8 gap-2">
                        <div>
                          ...
                          <h2 className="text-2xl sm:text-3xl font-black text-foreground mt-1">{selectedShipment.tracking_id}</h2>
                        </div>
                        <div className="sm:text-right">
                          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Live Location</p>
                          <p className="text-base sm:text-lg font-black text-primary uppercase mt-1">{selectedShipment.destination}</p>
                        </div>
                      </div>
          ...
          <motion.div key="camps" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex gap-2 sm:gap-3">
                <ExportDropdown data={camps.map(c => ({ Name: c.name, Location: c.location, Status: c.status, Occupancy: c.occupancy, Capacity: c.capacity, Lead: c.lead }))} filename="groundzero-camps" />
            ...
            {campsLoading ? (
              ...
              <motion.div variants={staggerContainer} initial="initial" animate="animate" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {camps.map(camp => (
                  <motion.div key={camp.id} variants={staggerItem} whileHover={{ scale: 1.02, boxShadow: '0 10px 40px rgba(0,0,0,0.08)' }} className="bg-card dark:bg-card-dark p-5 sm:p-6 rounded-3xl border border-border shadow-sm transition-all">
      ...
      case 'Resource Inventory':
        return (
          <motion.div key="inventory" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <h2 className="text-xl sm:text-2xl font-black text-foreground tracking-tight">Logistics & Supply</h2>
              <div className="flex gap-2 sm:gap-3">
                <ExportDropdown data={inventory.map(i => ({ Item: i.item_name, Category: i.category, Quantity: i.quantity, Unit: i.unit, Threshold: i.min_threshold, Critical: i.is_critical }))} filename="groundzero-inventory" />
                <button onClick={() => setNewEntryOpen(true)} className="bg-primary text-primary-foreground px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-primary/20 hover:opacity-90 active:scale-95 transition-all">
                  <span className="material-icons text-[18px]">add</span> <span className="hidden sm:inline">Add Item</span>
                </button>
              </div>
            </div>
            {inventoryLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left min-w-[500px]">
                    <thead className="bg-muted/50 border-b border-border">
                      <tr>
                        <th className="px-4 sm:px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Category</th>
                        <th className="px-4 sm:px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Item Name</th>
                        <th className="px-4 sm:px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest text-right">Stock Level</th>
                        <th className="px-4 sm:px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest text-right">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {inventory.map(item => (
                        <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-muted/50 transition-colors">
                          ...
                          <td className="px-4 sm:px-6 py-4 sm:py-5 text-xs font-black text-foreground">{item.item_name}</td>
      ...
      case 'Settings':
        return (
          <motion.div key="settings" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-8 max-w-2xl">
            <h2 className="text-xl sm:text-2xl font-black text-foreground tracking-tight">Preferences</h2>
  ...
  const getDefaultEntryType = () => {
    if (activeTab === 'Camps Management') return 'camp' as const;
    if (activeTab === 'Resource Inventory') return 'inventory' as const;
    if (activeTab === 'Global Logistics') return 'shipment' as const;
  ...
  return (
    <div className="flex h-screen bg-background-light dark:bg-background-dark overflow-hidden">
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[90] bg-black/40 backdrop-blur-sm lg:hidden" onClick={() => setSidebarOpen(false)} />
        )}
      </AnimatePresence>

      {/* Sidebar */}
      <aside className={`fixed lg:relative z-[100] w-72 bg-card dark:bg-card-dark border-r border-border flex flex-col shrink-0 h-full transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 sm:p-8 flex items-center gap-3 border-b border-border">
        ...
        <nav className="flex-1 p-4 flex flex-col gap-1 overflow-y-auto no-scrollbar">
          <SidebarNavItem active={activeTab === 'Dashboard'} icon="dashboard" label="Dashboard" onClick={() => { setActiveTab('Dashboard'); setSidebarOpen(false); }} />
          <SidebarNavItem active={activeTab === 'Camps Management'} icon="holiday_village" label="Camps" onClick={() => { setActiveTab('Camps Management'); setSidebarOpen(false); }} />
          <SidebarNavItem active={activeTab === 'Global Logistics'} icon="local_shipping" label="Logistics" onClick={() => { setActiveTab('Global Logistics'); setSidebarOpen(false); }} />
          <SidebarNavItem active={activeTab === 'Resource Inventory'} icon="inventory_2" label="Inventory" onClick={() => { setActiveTab('Resource Inventory'); setSidebarOpen(false); }} />
          ...
          <SidebarNavItem active={activeTab === 'Settings'} icon="settings" label="Settings" onClick={() => { setActiveTab('Settings'); setSidebarOpen(false); }} />
        </nav>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-14 sm:h-16 bg-card dark:bg-card-dark border-b border-border flex items-center justify-between px-4 sm:px-8 shrink-0 z-[50]">
          <div className="flex items-center gap-3 flex-1 max-w-lg">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden w-9 h-9 rounded-xl bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 max-w-[1400px] w-full mx-auto">
          <AnimatePresence mode="wait">
            {renderContent()}
          </AnimatePresence>
        </div>
...
const AdminStatCard: React.FC<{ icon: string; value: string; label: string; trend: string; trendColor: string; bgColor: string; iconColor: string; isUrgent?: boolean }> = ({ icon, value, label, trend, trendColor, bgColor, iconColor, isUrgent }) => (
  <motion.div variants={cardHover} initial="rest" whileHover="hover" className={`bg-card dark:bg-card-dark p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border ${isUrgent ? 'border-red-100 dark:border-red-900/50 ring-4 ring-red-50 dark:ring-red-950/5' : 'border-border'} shadow-sm cursor-default`}>
    <div className="flex justify-between items-start mb-4 sm:mb-6">
      <div className={`w-10 h-10 sm:w-12 sm:h-12 ${bgColor} ${iconColor} rounded-xl sm:rounded-2xl flex items-center justify-center shadow-sm`}>
        <span className="material-icons-round text-xl sm:text-2xl">{icon}</span>
      </div>
      <span className={`text-[8px] sm:text-[9px] font-black px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg ${trendColor} bg-muted border border-border`}>{trend}</span>
    </div>
    <p className="text-[9px] sm:text-[10px] font-black text-muted-foreground uppercase tracking-widest">{label}</p>
    <h3 className={`text-2xl sm:text-3xl font-black mt-1 tracking-tight ${isUrgent ? 'text-accent-red' : 'text-foreground'}`}>{value}</h3>
  </motion.div>
);
